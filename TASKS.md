...
## [ ] TASK-18: [BUG] Race condition in /api/models/start allows duplicate llama-server processes
...
* **Solution:** Not yet implemented. Options: (a) an in-process lock/flag in `app.py` around the pkill+Popen sequence so a second concurrent `/api/models/start` is refused (400) while one is already in flight, checking `is_open`/`is_open` status before proceeding; (b) coordinate with `~/start-translation-server.sh`'s own internal pkill+sleep, since that's the second half of the race.
* **Verification Method:** Reproduced via `curl -u vokov:0523 -X POST https://kindle.exodus.pp.ua/api/models/start` fired twice ~0.3s apart, then `ps aux | grep '[l]lama-server -m'` on the phone via SSH showed 2 PIDs both fully loaded and serving `/health`. Cleaned up manually and restored single-process baseline. Full report: ai-memory `notes/2026-07-15-models-manager-test-report.md` (project `kindle-butch-gen`).
* **Type:** `direct` (fix not yet applied)
