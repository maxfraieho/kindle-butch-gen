        ...
        <div className="flex items-center justify-between gap-3 border-b border-slate-800 pb-3">
          <div className="space-y-0.5 min-w-0 flex-1">
            <h2 className="text-base font-bold text-white flex items-center gap-2">
              ...
              <span className="truncate">Протокол обробки книги</span>
            </h2>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Badge variant={data?.mode === 'premium' ? 'amber' : 'slate'}>
              {data?.mode === 'premium' ? '⭐ Преміум' : '📦 Стандарт'}
            </Badge>
              ...
              return (
                <div key={stage.id} className="relative">
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-2.5 rounded-xl transition-all ${styles.bg} border ${styles.border} text-left hover:brightness-110`}
                  >
                    {/* Stage Number Circle */}
                    <div className={`w-[38px] h-[38px] rounded-full flex items-center justify-center shrink-0 border ${styles.border} ${styles.bg}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className={`text-sm font-semibold ${stage.status === 'completed' ? 'text-emerald-300' : stage.status === 'active' ? 'text-cyan-200' : 'text-slate-300'}`}>
                          {stage.name}
                        </span>
                        {stage.tier === 'premium' && (
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {stage.tier === 'standard' && stage.status !== 'active' && onRunStage && (
                        <Button
                          ...
                          className="px-2 py-1 h-7 text-[11px] text-cyan-300 hover:bg-cyan-950/60 border border-cyan-500/30"
                        >
                          {stage.status === 'completed' ? 'Перезапустити' : 'Запустити'}
                        </Button>
                      )}
                      <span className={styles.iconColor}>{ICON_MAP[stage.icon]}</span>
                      {isExpanded ? (
                  ...
                  {isExpanded && (
                    <div className={`ml-[19px] pl-6 pb-2 border-l-2 ${styles.line} mt-1 mb-1`}>
                      <div className={`p-3 rounded-xl ${styles.bg} border ${styles.border} space-y-2.5`}>
                        ...
                        {/* Direct action button in details */}
