...
export const StagesView: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'translated' | 'original' | 'autofixes' | 'cast'>('translated');
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800/60">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto no-scrollbar pb-0.5 shrink-0">
          <div className="glass-panel p-1 rounded-xl flex items-center gap-1 border border-slate-800 shrink-0 overflow-x-auto">
            <button
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all active-scale whitespace-nowrap flex items-center gap-1.5 ${
                activeTab === 'autofixes'
            ...
            >
              ...
              <span>⚡ Авто-правки та Препідготовка</span>
            </button>
            ...
            >
              ...
              <span>Персонажі ({characters.length})</span>
            </button>
      ...
      {/* Auto-Fixes & Audio Prep Visualizer Tab */}
      {activeTab === 'autofixes' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Card 1: Stressify */}
            <Card className="p-5 border-emerald-500/30 bg-emerald-950/10 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-emerald-300 font-bold text-sm">
                  <Sparkles className="w-4 h-4 text-emerald-400" />
                  <span>✨ Наголоси (Stressify)</span>
                </div>
                <Badge variant="emerald" size="sm">
                  {data?.cache_stats?.stressed_segments || 0} / {data?.total_chunks || data?.paragraphs?.length || 0}
                </Badge>
              </div>
              <p className="text-xs text-slate-300">
                Автоматична розстановка наголосів для точності українського синтезу мовлення (Supertonic 3 & StyleTTS2).
              </p>
              <div className="h-2 w-full bg-slate-900 rounded-full overflow-hidden border border-emerald-500/30">
                <div
                  ...
                  style={{
                    width: `${
                      data?.total_chunks
                        ? Math.min(100, Math.round(((data?.cache_stats?.stressed_segments || 0) / data.total_chunks) * 100))
                        : 0
                    }%`,
                  }}
                />
            ...
            {/* Card 2: Tone CoT Analysis */}
            <Card className="p-5 border-amber-500/30 bg-amber-950/10 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-amber-300 font-bold text-sm">
                  <FileText className="w-4 h-4 text-amber-400" />
                  <span>🎭 Аналіз тону (Tone CoT)</span>
                </div>
              ...
              <p className="text-xs text-slate-300">
                Ін'єкція емоційних тегів &lt;tone_analysis&gt; для адаптації темпу та динаміки озвучення.
              </p>
              <div className="text-[11px] font-mono text-slate-400 p-2.5 rounded-lg bg-slate-900/80 border border-slate-800">
                &lt;tone_analysis emotion="narrative" intensity="medium"&gt;
              </div>
            </Card>

            {/* Card 3: Cast & Gender Agreement */}
            <Card className="p-5 border-indigo-500/30 bg-indigo-950/10 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-indigo-300 font-bold text-sm">
                  <Users className="w-4 h-4 text-indigo-400" />
                  <span>🧬 Узгодження роду (Gemma 3)</span>
                </div>
              ...
              <p className="text-xs text-slate-300">
                Автоматична перевірка узгодження закінчень дієслів ("сказав" / "сказала") за реєстром персонажів.
              </p>
              <div className="flex items-center gap-2 text-xs">
                ...
                >
                  Перейти до реєстру персонажів →
                </Button>
              </div>
            </Card>

            {/* Card 4: Quality & ASR */}
            <Card className="p-5 border-cyan-500/30 bg-cyan-950/10 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-cyan-300 font-bold text-sm">
                  <ShieldAlert className="w-4 h-4 text-cyan-400" />
                  <span>🔍 Флаги якості & ASR Whisper</span>
                </div>
                <Badge variant={asrFlags.length + mqmFlags.length > 0 ? 'amber' : 'emerald'} size="sm">
                  {asrFlags.length + mqmFlags.length} проблем
                </Badge>
              </div>
              <p className="text-xs text-slate-300">
                Зворотний зв'язок Whisper ASR для виявлення спотворень або пропущених слів у згенерованому аудіо.
              </p>
            </Card>
          </div>

          {/* Pending Edits Approval Queue */}
          <Card className="p-5 border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-100 flex items-center gap-2">
                <Edit3 className="w-4 h-4 text-amber-400" />
                <span>Черга авто-правок та ручних змін ({pendingEdits.length})</span>
              </h3>
            </div>

            {pendingEdits.length === 0 ? (
              <p className="text-xs text-slate-400 py-4 text-center">Немає очікуючих правок для підтвердження.</p>
            ) : (
                  ...
                  >
                    <div className="space-y-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <Badge variant="amber" size="sm">{edit.mode}</Badge>
                        <span className="font-mono text-slate-400 text-[11px]">ID: {edit.target_id}</span>
                      </div>
                      <div className="text-rose-300/80 line-through">{edit.original_value}</div>
                      <div className="text-emerald-300 font-semibold">{edit.edited_value}</div>
                    </div>
      ...
      {/* Main Content Tabs (Translated / Original) */}
      {(activeTab === 'translated' || activeTab === 'original') && (
        <React.Fragment>
          {/* Paragraphs List */}
          {loading ? (
